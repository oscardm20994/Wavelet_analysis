...
__author__ = 'Oscar Dimdore-Miles'
...
def reconstruct(Y,wave, period, scale, dt, dj):
    Cdelta = 0.7785
    psi0 = np.pi**-0.25
    n = len(wave[0,:])# number of datapoints
    m = len(period) # number of periods
    reconst = [0]*n # make an empty array of the right length
    
    for i in range(0, n): # loop over datapoints
        for j in range(0, m): # loop over periods
            reconst[i] = reconst[i] + (((dt**0.5)*dj)/(Cdelta*psi0))* wave[j,i].real/(scale[j]**0.5)
    # the discrepanices between the orignial series and the reconstructed 
    variance_recon = np.std(reconst, ddof = 1)**2
    reconst = (reconst - np.mean(reconst))/np.sqrt(variance_recon) # normalised series
    variance_orig = np.std(Y, ddof = 1)**2
    reconst = reconst*np.sqrt(variance_orig) + np.mean(Y) # scale up to original series
    discrep = ((reconst - Y)/Y)*100                                                                           
